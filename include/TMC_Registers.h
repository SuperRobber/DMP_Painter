...
namespace TMC2130 {
  ...
  union GCONF { //Global Configuration
    uint32_t data;
  ...
  union GSTAT { //Global status flags
    uint32_t data;
  ...
  union CHOPCONF {   // Chopper and driver configuration
    uint32_t data;
  ...
  union IHOLD_IRUN { // Driver Current Control
    uint32_t data;
  ...
  union TPOWERDOWN { // Delay before power downl
    uint32_t data;
...
namespace TMC262 {
  ...
  union DRVCONF { // Driver Configuration
  ...
  union CHOPCONF { // Chopper Configuration
      uint8_t bytes[3]; // Raw value !Send byte 2 first, byte 0 last
      BitField<17,3> address;   // Register Address
  ...
  union SGCSCONF { //StallGuard2 and Current Settings
      uint8_t bytes[3]; // Raw value !Send byte 2 first, byte 0 last
      BitField<17,3> address; // Register Address
  ...
  union SMARTEN { // CoolStep Configuration
      uint8_t bytes[3]; // Raw value !Send byte 2 first, byte 0 last
      BitField<17,3> address;   // Register Address
